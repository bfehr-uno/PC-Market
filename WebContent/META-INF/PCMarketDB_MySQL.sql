CREATE TABLE Part_Type (id INT NOT NULL AUTO_INCREMENT,
						...
						FOREIGN KEY (Motherboard_id) REFERENCES Motherboard_Part(id),
						FOREIGN KEY (CPU_id) REFERENCES CPU_Part(id),
						FOREIGN KEY (Hard_Drive_id) REFERENCES Hard_Drive_Part(id),
						PRIMARY KEY (ID));
...
INSERT INTO GPU (id, Manufacturer, Model_Name, Port_Type, Interface, Memory_Size) values (default, ?, ?, ?, ?, ?);
...
SELECT M.Manufacturer, M.Model_Name, M.Socket, M.Form_Factor, 
	   ...
	   G.Manufacturer, G.Model_Name, G.Port_Type, G.Interface, G.Memory_Size,
